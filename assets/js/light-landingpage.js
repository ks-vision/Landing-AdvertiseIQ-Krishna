...
bloomPass.strength = 0; // Brighter glow for vivid colors
...
function initGlobe(imgData, imgWidth, imgHeight) {
    ...
    const colorWhite = new THREE.Color(0x020617); // Pure Black
    const colorLightBlue = new THREE.Color(0x020617); // Dark Blue
    const colorBlue = new THREE.Color(0x020617); // Royal Blue
    const colorDeepBlue = new THREE.Color(0x020617); // Deep Navy
    ...
    for (let i = 0; i < actualParticleCount; i++) {
        ...
        let mixH = hNorm;
        rocketCustomColors[i * 3] =
            (c_dark[0] + (c_light[0] - c_dark[0]) * mixH) * zDim;
        rocketCustomColors[i * 3 + 1] =
            (c_dark[1] + (c_light[1] - c_dark[1]) * mixH) * zDim;
        rocketCustomColors[i * 3 + 2] =
            (c_dark[2] + (c_light[2] - c_dark[2]) * mixH) * zDim;
    ...
    pointGeometry.setAttribute(
        "rocketPosition",
        new THREE.BufferAttribute(rocketPositions, 3),
    );
    pointGeometry.setAttribute(
        "rocketColorAttr",
        new THREE.BufferAttribute(rocketCustomColors, 3),
    );
    pointGeometry.setAttribute(
        "targetPosition",
    ...
    pointGeometry.setAttribute(
        "isOcean",
        new THREE.BufferAttribute(isOceanArr, 1),
    );
    ...
    for (let i = 0; i < maxPoints; i++) {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            ...
            let dy =
                tempTargetPos[idx1 * 3 + 1] - tempTargetPos[tempIdx * 3 + 1];
            let dz =
                tempTargetPos[idx1 * 3 + 2] - tempTargetPos[tempIdx * 3 + 2];
        ...
        netPts.push(
            tempTargetPos[idx1 * 3],
            tempTargetPos[idx1 * 3 + 1],
            tempTargetPos[idx1 * 3 + 2],
        );
        netPts.push(
            tempTargetPos[idx2 * 3],
            tempTargetPos[idx2 * 3 + 1],
            tempTargetPos[idx2 * 3 + 2],
        );
    ...
    netGeo.setAttribute(
        "position",
        new THREE.Float32BufferAttribute(netPts, 3),
    );
    ...
    exhaustGeo.setAttribute(
        "position",
        new THREE.BufferAttribute(exhaustPts, 3),
    );
    exhaustGeo.setAttribute(
        "phase",
        new THREE.BufferAttribute(exhaustPhases, 1),
    );
...
floorGeo.setAttribute(
    "originalY",
    new THREE.Float32BufferAttribute(floorOriginalY, 1),
);
...
function initGSAP() {
    ...
    morphTl.to(
        starMat.uniforms.opacity,
        { value: 0, duration: 2, ease: "power1.inOut" },
        0,
    );
    morphTl.to(
        floorMat.uniforms.opacity,
        { value: 0, duration: 2, ease: "power1.inOut" },
        0,
    );
    morphTl.to(
        glowMat.uniforms.opacity,
        { value: 0, duration: 2, ease: "power1.inOut" },
        0,
    );
    ...
    morphTl.to(
        globeGroup.position,
        { x: 0, duration: 2, ease: "power1.inOut" },
        0,
    );
    morphTl.to(
        globeGroup.rotation,
        { z: 0, duration: 2, ease: "power1.inOut" },
        0,
    );
    ...
    if (netMat)
        morphTl.to(
            netMat,
            { opacity: 0.0, duration: 2, ease: "power2.in" },
            0.5,
        );
    ...
    section3Tl.fromTo(
        glowMat.uniforms.opacity,
        { value: 0.0 },
        { value: 1.0, duration: 2, ease: "power1.inOut" },
        0,
    );
    ...
    contentTl
        .to(".ai-operations-section .ai-header", {
            opacity: 1,
            y: 0,
            duration: 1.2,
            ease: "power3.out",
    ...
    if (pointMaterial && pointMaterial.uniforms) {
        ...
        rocketMorphTl.to(
            pointMaterial.uniforms.masterOpacity,
            { value: 0.0, duration: 2.5, ease: "power1.inOut" },
            0,
        );
    }
